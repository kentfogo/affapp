...
export default function SessionScreen() {
  ...
  useEffect(() => {
    if (currentSession) {
      ...
      console.log('Session affirmations played:', currentSession.affirmationsPlayed.length);
      console.log('Session is active:', currentSession.isActive);
      console.log('=============================');
    }
  }, [currentSession]);
  ...
  const initializeSession = async () => {
    try {
      ...
      console.log('=== PRE-SESSION STATE ===');
      console.log('Previous session affirmations:', currentSession?.affirmationsPlayed.length || 0);
      console.log('Current state index:', currentAffirmationIndex);
      console.log('========================');
      ...
      // ✅ This should create fresh session with affirmationsPlayed: []
      startSession();
      
      // Debug: Verify session was reset
      console.log('=== POST-SESSION STATE ===');
      console.log('New session affirmations:', currentSession?.affirmationsPlayed.length);
      console.log('New state index:', currentAffirmationIndex);
      console.log('=========================');
  ...
  const triggerAffirmation = async () => {
    ...
    // Get latest session state synchronously to avoid stale closures in intervals
    const latestSession = useSessionStore.getState().currentSession;

    console.log(
      '🔵 TRIGGER START - Current played count:',
      latestSession?.affirmationsPlayed.length || 0
    );
    ...
    // Get current played affirmations from latest store state (avoids stale closure)
    const currentPlayed = latestSession?.affirmationsPlayed || [];
    // Only add if this affirmation hasn't been added yet (prevents duplicates)
    const newAffirmationsPlayed = currentPlayed.includes(affirmation.id) 
      ? currentPlayed  // Already added, don't add again
      : [...currentPlayed, affirmation.id];  // Not added yet, add now

    console.log('📊 Affirmations count:', currentPlayed.length, '→', newAffirmationsPlayed.length);
